(function( $ ){
    $.fn.bootpage = function(options) {
        ...
        var api = {
             ...
             * may referer a more than one group og thumbnails, each with his
            ...
            renderGroup: function($el) {
                ...
                // sort only the first time and when there are a filter or tag
                // selection
                var init = $el.find('.bp-filter-container').length;
                if (init === 0) {
                    this.sort($el);
                }
                ...
                // construnct one new
                $el.find('.bp-pagination-wrapper').remove();
                ...
                if (options.pagination) {
                    this.setUpPagination($el, pages);
                }

                if (options.filter && init === 0) {
                    this.setUpFilter($el);
                }
             ...
             */
            setUpPagination: function($el, pages) {
                // exit if no pages
                if (!pages) return;
                ...
                var $listWrapper = $('<div>')
                    ...
                    .addClass('text-' + options.paginationPosition)
                    // todo: make this class configurable
                    .addClass('col-md-12')
                    .addClass('bp-pagination-wrapper');
                var $list = $('<ul>').addClass(options.paginationClass);
                // add the pagination size, pagination-xs/sm/lg
                if (options.paginationSize) {
                    $list.addClass(options.paginationSize);
                }

                for (i=1; i<=pages; i+=1) {
                    var $li = $('<li data-index="'+i+'"><a href="#">'+i+'</a></li>');
                    if (i === options.currentPage) {
                        $li = $('<li data-index="'+i+'" class="active"><a href="#">'+i+'</a></li>');
                    }
                    $li.on('click', this.showPage);
                    $list.append($li);
                }

                var all = options.i18n.all;
                var $liall = $('<li data-index="'+all+'"><a href="#">'+all+'</a></li>');
                $liall.on('click', this.showPage);
                $list.append($liall);

                // add paginator to element
                $listWrapper.append($list);
                $el.append($listWrapper);
            ...
            setUpFilter: function($el) {
                ...
                if ($el.find('.bp-filter-input').length > 0) {
                    return;
                }

                // HTML elements and actions
                var $col = $('<div>')
                    // add some margin
                ...
                var $input = $('<input>')
                    .addClass('form-control bp-filter-input')
                    .attr('type', 'text')
                    .focus()
                    .on('keyup', function() {
                ...
                $group.append($span);
                $col.append($group);
                $el.find('.bp-filter-container').prepend($col);
             ...
             * into the top filter container. Also create the click event on
             * each tag.
             *
             ...
             * uses de event fired by jQuery when click a link or the active
             * page.
             *
            ...
            showPage: function($el) {
                ...
                if ($el.currentTarget) {
                    ...
                    $el = $el.find('.' + options.paginationClass).find('li.active');
                }

                var $EL = $el.closest(selector);
                $EL.find(options.itemsClass).hide();

                $el.siblings('li').removeClass('active');
                $el.addClass('active');
                options.currentPage = parseInt($el.find('a').html(), 10);
                ...
                var i, start = options.currentPage * options.perpage - 1;
                var end = start + options.perpage - 1;
                for (i = start; i<=end; i+=1) {
                    $EL.find(options.itemsClass).not('.hide').eq(i-1).fadeIn('fast');
                }
